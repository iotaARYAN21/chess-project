...
async def handle_resolve_log(payload: ResolveRequest,admincred = Depends(AdminChecker(AdminLevel.MODERATOR))):
...
async def check_user_bans(account_id: uuid.UUID):
...
async def check_if_banned(account_id: uuid.UUID):
...
async def report_suspicious_activity(user_id: uuid.UUID, match_id: uuid.UUID, sus_score: float):
